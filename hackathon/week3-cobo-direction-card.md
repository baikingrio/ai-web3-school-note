...
> Project status: Cobo CAW 最小接入为 Week 4 主路径；Safe + Zodiac Roles 为已跑通 fallback；已加入 6/1 VC 视角校准原则。
...
1. **不做泛泛的“AI 钱包”**：只解决 Agent 支付 / 资源采购里的权限边界问题。
2. **AI 只负责建议**：Agent 生成结构化付款建议；授权必须来自 policy hard-check / CAW Pact / Safe Roles。
3. **Web3 要有必要性**：用链上权限、tx hash、audit log、撤销机制证明“可验证、可追踪、可收回”，不是为了包装概念。
4. **MVP 不堆功能**：优先证明额度内成功、超额拒绝、非白名单拒绝、撤销后失败。
5. **真实场景优先**：默认场景固定为“Agent 为 builder 购买白名单 API / 数据 / 工具服务”。
6. **透明说明集成状态**：CAW 是目标执行层；如果接入受阻，Safe + Zodiac Roles 是 fallback，不能包装成已完成 CAW 集成。
7. **Pitch 顺序**：问题 → 目标用户 → 最小场景 → Policy 边界 → Cobo / fallback 执行 → 四条 Demo → 下一步。

一句话防偏：**不做大而全的钱包平台，只做 Agent 受限支付 / 资源采购的可验证最小闭环。**
...
- Day 1：整理 README / demo script，统一 Cobo 赛道语言；开头先讲“Agent 不能拥有无限钱包权限”的用户问题。
...
- Day 6：录制 3–5 分钟 Demo 视频，按“问题 → 方案 → 四条安全路径”组织。
- Day 7：最终检查 README、提交包、安全边界、CAW / fallback 表述和 Hackathon submission。
