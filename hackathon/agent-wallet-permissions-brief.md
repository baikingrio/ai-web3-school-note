...
> 学员：Quinn · AI 有基础 · Web3 熟悉 · 能独立开发 · Week 3 起每日约 2–3 小时  
> 状态：v0.4 Safe / Zodiac Roles 已跑通；Week 4 主线切到 Cobo CAW 最小接入；已吸收 6/1 VC 视角打磨反馈 · 2026-06-02
...
这次 Hackathon 后续设计和开发默认按以下原则收敛，避免走偏：

1. **不要把项目讲成泛泛的“AI 钱包”**：核心问题是“AI Agent 可以提出付款 / 采购建议，但不能获得无限钱包权限”。所有功能都要服务于这个权限边界问题。
2. **AI 是建议层，不是授权层**：LLM 的价值是理解任务、生成结构化建议、解释结果；是否执行只能由 policy hard-check / CAW Pact / Safe Roles 决定。
3. **Web3 必须有必要性**：链上钱包、Pact / Roles、tx hash、audit log 的价值在于可验证权限、可撤销授权和可追踪执行，而不是为了“用了链”而用链。
4. **先证明真实场景，不堆功能**：Demo 只围绕“Agent 购买白名单 API / 数据 / 工具服务”展开；每个页面、脚本、合约或工具都要能帮助评委看懂这个场景。
5. **MVP 以四条安全路径为中心**：额度内成功、超额拒绝、非白名单拒绝、撤销后失败是必须拍清楚的主线；pending approval / human check 是加分，不替代主线。
6. **诚实透明地呈现集成状态**：CAW 是目标执行层；如果 SDK / API 权限或测试环境受阻，就明确说 Safe + Zodiac Roles 是 fallback，不包装成已完成 CAW 集成。
7. **Pitch 先讲问题和用户，再讲技术**：先说明 builder 为什么不敢给 Agent 主私钥 / 无限授权，再讲 AgentScoope 如何用预算、白名单、人工确认和审计解决这个问题。

一句话防偏：

> 不做“大而全的 AI 钱包平台”，只做“Agent 受限支付 / 资源采购”的可验证最小闭环。
...
MVP 判断标准：如果某个功能不能帮助评委更快理解“Agent 在权限边界内执行付款，越界会被拒绝或要求人工确认”，就先不做。
...
- 完整 marketplace、复杂 escrow、仲裁系统、评价系统、分润系统
- 为了展示技术复杂度而新增的合约、页面或流程；没有帮助说明权限边界的功能全部延后

### 防走偏检查清单

后续每次新增设计 / 代码 / Demo 前，先问这 6 个问题：

1. 这个功能是否直接服务于“受限执行”这个核心问题？
2. 它是否能增强四条 Demo 主线之一：成功、超额拒绝、非白名单拒绝、撤销失败？
3. 它是否让 Agent 和 Policy / Wallet 的责任边界更清楚，而不是更模糊？
4. 它是否能在 Week 4 时间内稳定演示，而不是增加不确定性？
5. 它是否能被 audit log / tx hash / denial reason 验证？
6. 如果去掉它，评委是否仍能理解项目价值？如果答案是“能”，先去掉。

优先级排序：**清楚问题 > 可跑 Demo > 可验证安全边界 > Cobo 赛道贴合 > UI 好看 > 扩展功能**。
...
1. 把 README / Demo script 改成 Cobo 赛道语言：Agent-Native Payments + Resource Procurement；开头先讲用户不敢给 Agent 无限钱包权限的问题。
2. 尝试 CAW SDK / CLI 最小接入：allowed transfer、pending approval、structured denial 至少跑通 1–2 条。
3. 如果 CAW 接入受阻，保留 Safe + Zodiac Roles fallback，但在提交材料里透明说明，不夸大集成状态。
4. 录制 3–5 分钟 Demo：额度内成功、超额拒绝、非白名单拒绝、撤销后失败；每条都展示 policy decision + audit log / tx hash / denial reason。
5. 准备 Pitch：问题 → 目标用户 → 最小场景 → Policy 边界 → Cobo / fallback 执行 → 四条 Demo → 下一步。
6. v0.5 以后再考虑：多 token / 链上日累计 allowance / bounty payout / escrow / marketplace。
